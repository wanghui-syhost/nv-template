<template>
    <div class="upload-table">
        ...
        <!-- 文件列表 -->
        <el-table :data="list" ref="multipleTable"  @selection-change="handleSelectionChange" fit highlight-current-row height="500">
                ...
                <template slot-scope="scope">
                    <span class="file-name">
                        ...
                        <span class="file-input" v-show="scope.row.isEdit">
                            ...
                            <span style="float: left; margin-left: 20px"><el-button type="primary" size="small" @click="modifyComplete(scope.row)" :disabled="rename.trim().length<1">确认</el-button></span>
        ...
        <el-dialog title="新建文件夹" :visible.sync="dialogFormVisible">
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="confirmToAddFolder" :disabled="newFolderName.trim().length<1">确 定</el-button>
            </div>
        </el-dialog>
    </div>
</template>
        ...
        FileRename(data).then(resp=>{
          ...
          if(code==0){
            ...
            if(row.NAME.indexOf(".") < 0){
              row.NAME = row.NAME + row.FILE_TYPE;
            }
            row.isEdit = false;
            me.$message({
              showClose: true,
