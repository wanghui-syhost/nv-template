<template>
    <el-table :data="files" ref="multipleTable" @selection-change="handleSelectionChange" fit highlight-current-row>
        ...
        <el-table-column label="" width="180">
            <template slot-scope="scope">
                <i title="重命名" class="png-icon cursor file-rename small" @click="handleReName(scope.row)" v-if="scope.row.FILE_STATUS==3 ||  scope.row.FILE_STATUS==0 || scope.row.IS_DIRECTORY == 'YES'"></i>
                <i title="下载" class="png-icon cursor file-upload  small" @click="download(scope.row)"></i>
                <i title="删除" class="png-icon cursor file-delete small" @click="removeItem(scope.row)" v-if="scope.row.FILE_STATUS==3 || scope.row.FILE_STATUS==0 || scope.row.IS_DIRECTORY == 'YES'"></i>
                <i title="移动到" class="png-icon cursor file-txt small" @click="moveFolderTo(scope.row)" v-if="scope.row.IS_DIRECTORY == 'NO' && scope.row.FILE_STATUS==3 || scope.row.IS_DIRECTORY == 'NO' && scope.row.FILE_STATUS==0"></i>
                <i title="移交" class="png-icon cursor file-handon small" @click="handOnToDialog(scope.row)" v-if="userName==scope.row.CREATE_USER && scope.row.IS_DIRECTORY == 'NO' && scope.row.FILE_STATUS==0 || userName==scope.row.CREATE_USER && scope.row.IS_DIRECTORY == 'NO' && scope.row.FILE_STATUS==3"></i>
                <i title="审核" class="png-icon cursor file-folder small" @click="reviewedDialog(scope.row,1)" v-if="scope.row.IS_DIRECTORY == 'NO'  && scope.row.FILE_STATUS==1 && userName==scope.row.RECIPIENT"></i>
            </template>
        </el-table-column>
    </el-table>
</template>
...
export default {
    ...
    methods: {
        ...
        handleReName (file) {
            ...
            self.newName = file.IS_DIRECTORY == "YES" ? file.NAME : file.NAME.substring(0, row.NAME.lastIndexOf("."));
            file.isEdit = true;
        },
