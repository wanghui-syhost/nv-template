<template>
    <div class="upload-table" v-loading.body="isUploading" element-loading-text="正在上传中，请稍等......">
        <section class="upload-table__from">
            ...
            <el-button @click="createdNewFolder" type="primary" plain size="mini" :disabled ="isSearch" v-show="true">
                新建文件夹
            </el-button>
            <el-button @click="downloadChooseRows" type="primary" plain size="mini" v-show="isShowDownload">
                下载
            </el-button>
            <el-button @click="deleteSelectedAndChildren" plain type="primary" size="mini" v-show="isShowDelete">
                删除
            </el-button>
            <el-button   @click="batchHandOnToDialog()" plain  size="mini" v-show="levelList.length" type="primary">
                移交
            </el-button>
            ...
            <el-upload  class="upload-table__upload--btn" :action="uploadURL" 
              ...
              accept=".jpg, .jpeg, .png, .gif, .rar, .zip, .doc, .docx, .xls, .xlsx,  .ppt, .pptx, .pdf, .txt, .wps">
                <el-button  type="primary" plain v-show="levelList.length > 1 && showUpload && isShowUpload">上传</el-button>
            </el-upload>

            <div style="float: right">
              <el-input style="width:300px" icon="search"
                  placeholder="请输入关键字" 
                  v-model="fileName" 
        ...
        <div class="uploader_main" :class="componentType">
          <component 
            :is="componentType"
            :files="list"
            :user-name = "userName"
...
export default {
  ...
  computed: {
    isAllSelected: {
      ...
      set (val) {
        ...
        list.forEach(file => (file.isChecked = val))
        return isDispayBlock || this.$refs.componentType.updateSelection()
      }
    },
    componentType() {
    ...
    selectedIds() {
      ...
      return selectedFiles.map(item => {
          return item.IS_DIRECTORY === "YES"
            ? `DIR-${item.TREE_ID}`
            : `FILE-${item.TREE_ID}-${item.ID}`;
        })
        .join(",");
    },
    treeId() {
  ...
  methods: {
    ...
    batchHandOnToDialog() {
      const self = this;
      self.selectedFiles.forEach(item => {
        ...
        if (
          "NO" === item.IS_DIRECTORY &&
          (item.FILE_STATUS === "2" || item.FILE_STATUS === "1")
        ) {
    ...
    adoptConfirm(fileStatus) {
      ...
      const arr = {
        ...
        FILE_ID: self.reviewedForm.ID,
        OPINION: self.reviewedForm.OPINION
      };
      //let param={dataList: JSON.stringify(arr)};
      adoptSave(arr)
        .then(({ data }) => {
    ...
    download(row) {
      unfetch.download("file/download/compress", {
        params: {
          ID:
            row.IS_DIRECTORY === "YES"
              ? `DIR-${row.TREE_ID}`
              : `FILE-${row.TREE_ID}-${row.ID}`
        }
      });
    },
    refresh () {
    ...
    deleteSelectedAndChildren() {
      ...
      me
        ...
        .then(() => {
          deleteDirAndFiles(me.selectedIds)
            .then(() => {
              me.$message.success("删除成功");
              me.fetchData(me.currentId);
            })
